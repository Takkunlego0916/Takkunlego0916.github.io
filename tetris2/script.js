...
import { Play, Pause, RotateCcw, ChevronDown, ChevronLeft, ChevronRight, RotateCw } from 'lucide-react';
            ...
            {row.map((cell, x) => (
              <div
                ...
                className={`w-4 h-4 sm:w-5 sm:h-5 rounded-sm ${
                  cell ? `${nextPiece.color} shadow-md` : 'bg-transparent'
                }`}
              />
            ))}
          </div>
              ...
              {renderBoard().map((row, y) => (
                <div key={y} className="flex gap-0.5">
                  {row.map((cell, x) => (
                    <div
                      key={x}
                      className={`w-5 h-5 sm:w-6 sm:h-6 rounded-sm transition-all duration-75 ${
                        cell 
                          ? `${cell} shadow-lg` 
          ...
          <div className="lg:hidden bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
            <div className="grid grid-cols-3 gap-2">
              ...
              <Button 
                ...
                onClick={rotatePiece}
                className="h-12 w-12 mx-auto border border-slate-700"
              >
              ...
              <Button 
                ...
                onClick={hardDrop}
                className="h-12 w-12 mx-auto border border-slate-700 bg-slate-700/50"
              >
